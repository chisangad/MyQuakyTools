...
#' @param path path to output folder of analysis (e.g. PATH/my_analysis.GseaPreranked.1470948568349)
#' @param gene.set name of the gene set (e.g. V$AP1_Q2).
...
#' @param menrichment.score.range the range of the enrichment score; defaults to [min(ENRICHMENT SCORE), max(ENRICHMENT SCORE)]
#' @export
#' @details
#' replotGSEA re-plots data from the javaGSEA desktop version in R.
#' The function takes three arguments: path, the path to the javaGSEA output folder; gene.set,
#' which is the name of the gene set you want to plot (note: approximate matching is used in a grep-function to determine the appropriate gene set); and class.name, which is the name of the variable to which gene expression has been matched (i.e., the description of the variable that is specified in the .cls file or that is used to pre-rank the data; example: "drug treatment").
